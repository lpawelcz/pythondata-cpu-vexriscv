...
object GenCoreDefault{
  ...
  def main(args: Array[String]) {
    ...
    val parser = new scopt.OptionParser[ArgConfig]("VexRiscvGen") {
      ...
      opt[Int]("hardwareBreakpointCount")    action { (v, c) => c.copy(hardwareBreakpointCount = v)   } text("Set number of hardware breakpoints, 0 means none, only valid when debug enabled")
      // ex : -iCacheSize=XXX
